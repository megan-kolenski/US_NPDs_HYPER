...
def Cost_OakRidge(design_ic, design_h, Q_design):
    """
    Calculate the cost of powering based on design parameters.

    Parameters:
        design_ic : Installed capacity (kW)
        design_h  : Design head (m)
        Q_design  : Design discharge (m^3/s)

    Returns:
        COST : Estimated cost (in millions, presumably)
    """

    # Convert to imperial units
    f_Q_design = Q_design * 35.3147
    f_design_h = design_h * 3.28084
    f_design_ic = design_ic / 1000 
    
    #f_Q_design = design_ic/9.81/design_h/0.9 * 35.3147
    # Head correction factor
    if f_design_h <= 30:
        dummy_head = -0.125106425
    elif f_design_h <= 60:
        dummy_head = -0.175941187
    elif f_design_h <= 100:
        dummy_head = -0.127623996
    else:
        dummy_head = 0 

    # Installed capacity correction factor
    if f_design_ic <= 1:
        dummy_P = -0.222836146
    elif f_design_ic <= 10:
        dummy_P = -0.191883427
    ...
    COST = (1.23 * 1.26550518 * 
            np.exp(
            0.701421442 +
            0.876738864 * np.log(f_Q_design) +
            0.744231076 * np.log(f_design_h) +
            dummy_head  + dummy_P )
            ) / 1000
...
def interpolate_and_plot(vals, n=1000, plot_title='Fitted Curve'):
    """
    Interpolates given values using PCHIP over uniform percentiles and plots the result.

    Parameters:
        pct (array-like): Percentiles (e.g., [0, 10, ..., 100]).
        vals (array-like): Corresponding values at each percentile.
        n (int): Number of uniform points to interpolate over. Default is 1000.
        plot_title (str): Title for the plot.

    Returns:
        uniform_pct (np.ndarray): Uniform percentiles (0 to 100).
        uniform_vals (np.ndarray): Interpolated values at uniform percentiles.
    """
    ...
    pchip = PchipInterpolator(pct, vals, extrapolate=False)

    # Uniform percentiles
    uniform_pct = np.linspace(0, 100, n)
    uniform_vals = pchip(uniform_pct)

    # Plot
    plt.figure(figsize=(8, 5))
    plt.plot(uniform_pct, uniform_vals, linewidth=2, label=f'PCHIP (0–100%, {n} pts)')
    plt.scatter(pct[1:-1], vals[1:-1], color='red', s=50, label='Original deciles (10–90%)')
    plt.scatter([pct[0], pct[-1]], [vals[0], vals[-1]],
                color='black', marker='x', s=80, label='Known min/max')
